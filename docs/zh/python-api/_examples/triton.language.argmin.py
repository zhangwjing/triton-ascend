...
def test_argmin():
    ...
    triton_argmin_2d[1, 1, 1](x, out, dim, M, N, M, N)
    ref = torch.argmin(x, dim=dim).to(torch.int32)
    assert torch.equal(out.cpu(), ref.cpu()), "argmin result mismatch"
    print("argmin result:", out)
